...
void *database_worker(void *arg)
{
    ...
    for (int i = 0; i < UPDATES_PER_WORKER; i++) {
        ...
        pthread_mutex_lock(&records[record_index].mutex);
        ...
        pthread_mutex_unlock(&records[record_index].mutex);
    }
...
int main(void)
{
    ...
    printf("=====================================\n");
    printf("Phase 2: Synchronized Database Updates\n");
    printf("=====================================\n\n");
    ...
    for (int i = 0; i < NUM_RECORDS; i++) {
        ...
        if (pthread_mutex_init(&records[i].mutex, NULL) != 0) {
            fprintf(stderr, "Failed to initialize mutex for record %d\n", i + 1);
            return EXIT_FAILURE;
        }
    }
    ...
    for (int i = 0; i < NUM_RECORDS; i++) {
        ...
        if (records[i].value == expected_per_record &&
            records[i].update_count == expected_per_record) {
    ...
    if (actual_total == expected_total) {
        printf("\nMutex synchronization worked successfully.\n");
    } else {
        printf("\nUnexpected synchronization error occurred.\n");
    }
